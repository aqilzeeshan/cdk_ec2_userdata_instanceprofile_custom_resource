...
import {Fn, Tags, Resource} from '@aws-cdk/core';
...
export class CdkEc2UserdataInstanceprofileCustomResourceStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
    super(scope, id, props);

    // create VPC with public and private subnets in 1 AZ
    // this also creates a NAT Gateway
    // 1 AZ is being used because this is an example (not production)
    // In production always use >= 2 AZ's
    ...
    const role = new Role(this, 'MyAppRole',{
      assumedBy: new ServicePrincipal('ec2.amazonaws.com')
    })
    //arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore
    role.addManagedPolicy(ManagedPolicy.fromAwsManagedPolicyName('AmazonSSMManagedInstanceCore'))

    //define a user data script to install & launch web server
    ...
    ssmaUserData.addCommands('yum install -y nginx','chkconfig ngingx on','service ngingx start')

    //launch an EC2 instance in the private subnet
    const instance = new Ec2(this,'MyAppInstance',{
...
/**
 * Create your own EC2 resource and EC2 props as these are not yet defined in CDK
 * These classes abstracts low level details from CloudFormation
 */
...
class Ec2 extends Resource{
  constructor(scope: cdk.Construct, id: string, props?: Ec2InstanceProps){
    ...
    if(props){
      ...
      //tag the instance
      Tags.of(this).add('Name', `${CdkEc2UserdataInstanceprofileCustomResourceStack.name}/${id}}`);
